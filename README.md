...
- It supports only reach set of locales for simplify and minimize size. (But works with another languages to translate pagaes for ex and to show user interface). So more complex implementation may be not requered.
The current implementation _requeres to manually change the language from Settings_. (issue #2) Fixed: now locales detected automatically (I hope). If you want another locale or have issue with it, write to this issue: #2
